...
?>
		...
		<!-- </div> -->
		<?php 
		$excel_name=$_LANG_TEXT['scanfiletext'][$lang_code];
		?>
		<div class="btn_wrap right">
			<? $excel_down_url = $_www_server."/result/get_scan_file_excel.php?enc=".ParamEnCoding($param);
						?>
			<div class="right">
				<a href="javascript:void(0)" id="file_scan_list_excel"  title="<?echo trsLang('검사파일','scanfiletext')." ".$_LANG_TEXT["btnexceldownload"][$lang_code]; ?>" class="btnexcel required-print-auth hide" onclick="getHTMLSplit('<?= $total ?>','<?= $excel_down_url ?>','<?= $excel_name ?>',this);"><?= $_LANG_TEXT["btnexceldownload"][$lang_code]; ?></a>
			</div>
		</div>
