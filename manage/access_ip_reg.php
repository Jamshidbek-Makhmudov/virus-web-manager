		...
		<form name="frmAccessIP" id="frmAccessIP" method="post">
			...
			<table class="view">
				<tr>
					...
					<td>
						<input type="text" name="ip_addr" id="ip_addr" class="frm_input" value="<?php echo $ip_addr; ?>" style="width:280px" maxlength="20">
					</td>
				</tr>
				<tr class="bg">
					...
					<td>
						<select name='allow_id' id="allow_id" class='search_select' style="width:300px;">
							<option value="" <?if($allow_id=="" ) echo 'selected="selected"' ;?>><? echo $_LANG_TEXT['choosetext'][$lang_code]?></option>
							<option value="ALL" <?if($allow_id=="ALL" ) echo 'selected="selected"' ;?> ><? echo $_LANG_TEXT['alltext'][$lang_code]?></option>
							<?php
							...
							?>
							<option value="<?php echo $_admin_id ?>" <?php if($_admin_id == $allow_id) { echo ' selected="selected"'; } ?>><?php echo $_admin_name; ?> (<?= $_admin_id ?>)</option>
							<?php
							}
							?>
						</select>
