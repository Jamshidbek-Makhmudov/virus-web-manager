...
<div id="oper_list">
	<div class="container">
		...
		<div id="tit_area">
			<div class="tit_line">
				<h1><span id='page_title'><?=$_LANG_TEXT['m_manage_admin.info'][$lang_code]?></span></h1>
			</div>
		...
		<tr>
			...
			<td>
				<select name="searchopt" class="select_bg" id="searchopt">
					...
					<option value="EMP_NAME" <?php if($searchopt == "EMP_NAME") { echo ' selected="selected"'; } ?>><?=$_LANG_TEXT["adminnametext"][$lang_code];?></option>
					<option value="EMP_NO" <?php if($searchopt == "EMP_NO") { echo ' selected="selected"'; } ?>><?=$_LANG_TEXT["empnotext"][$lang_code];?></option>
		...
		<div class="btn_wrap" >
			<? 
				...
				$excel_down_url = $_www_server."/manage/admin_list_excel.php?enc=".$param_enc;
			?>
			<div class="right">
		...
		<tr>
			...
			<th style='min-width:90px'><?=$_LANG_TEXT["adminnametext"][$lang_code];?></th>
			...
			<a href="javascript:void(0)" onclick="sendPostForm('<?=$PHP_SELF?>?enc=<?=ParamEnCoding($param.($param ? "&" : "")."orderby=".($orderby=="org_name"? "org_name desc" : "org_name"))?>')"  class='sort'>
			<?=$_LANG_TEXT["workplacetext"][$lang_code];?></a></th>
			<th style='min-width:100px'><a href="javascript:void(0)" onclick="sendPostForm('<?=$PHP_SELF?>?enc=<?=ParamEnCoding($param.($param ? "&" : "")."orderby=".($orderby=="dept_name"? "dept_name desc" : "dept_name"))?>')"  class='sort'><?=$_LANG_TEXT["depttext"][$lang_code];?></a></th>
		</tr>
	<?php
			 ...
			 if($result){
			  while($row=sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC)){
					...
					if($_encryption_kind=="1"){
						...
						$email = (empty($row['email'])) ? "" : aes_256_dec($row['email']);
						$phone_no = (empty($row['phone_no'])) ? "" : aes_256_dec($row['phone_no']);
					}
			  ...
			  ?>	
				<tr onclick="sendPostForm('./admin_reg.php?enc=<?=$param_enc?>')" style='cursor:pointer'>
					<td><?php echo $no; ?></td>
					<td><?=$emp_name?></td>
					<td><?=$emp_no?></td>
...
?>
		<div class="btn_confirm">
			<a href="./admin_reg.php" class="btn required-create-auth hide"><?=$_LANG_TEXT["btnregist"][$lang_code];?></a>
		</div>
