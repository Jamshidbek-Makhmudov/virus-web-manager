...
//		echo $visitor_id;
//		echo $phone_num;
//		echo AES_Rijndael_Decript('Cmgl9r11yju0sf1llI13Qw==', $_AES_KEY, $_AES_IV); 
//        exit;

//		echo $json_value['visit_num']."<BR>";
//		exit;

//		echo AES_Rijndael_Encript("220819", $_AES_KEY, $_AES_IV)."<BR>";
//		echo $json_value['visit_num']."<BR>";
//		exit;
...
//		exit;
		...
		if($v_user_seq > 0) {
			...
			if($os_ver_name =="Removable" || $os_ver_name =="HDD"){	
				
				//USB, HDD는 DPT 반입완료후 점검완료 처리를 한다. 
				$vcs_status = "";
			
			}else{ // CD-ROM은 여기서 완료를 해야 한다.
				...
				VALUES ( $wvcs_seq, '$sys_sn', '$hdd_sn', '$board_sn', '$model_name', '$manufacturer', '$host_name', $ram_size, '$mac_addr'
					...
					for($i=0; $i < count($arrDevNum); $i++) {
							...
							if($os_ver_name =="Removable" || $os_ver_name =="HDD"){	
								...
								$url = $_baseurl . "/wvcs/api/set_user_status.php?visit_num=$visit_num_param&visit_dev_num=$visit_dev_num_param&vcs_status=$vcs_status_param";
							}
