...
include "./common.php";
		...
		$sql_user = "	SELECT top 1 USER_INFO_LIST_SEQ, Replace( {$qry_Decrypt_PHONE}, '-','') as PHONE,  {$qry_Decrypt_EMAIL} as EMAIL
								, {$qry_Decrypt_USER_NAME} AS USER_NAME, COMPANY_NAME, {$qry_Decrypt_MANAGER_NAME} AS MANAGER_NAME, MANAGER_DEPARTMENT 
							FROM  DPT_USER_INFO_LIST
							WHERE Replace( dbo.fn_DecryptString(phone), '-','')='$phone_num' 
