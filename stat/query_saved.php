	...
	<div class="container">
		...
		<div id="tit_area">
			...
			<span class="line"></span>
				...
				<!--tab 메뉴-->
		<ul class="tab">
			<li  onclick="location.href='<? echo $_www_server?>/stat/query_editor.php'"><? echo trsLang('쿼리 검색 결과','query_search_result');?></li>
			<li class="on"  onclick="location.href='<? echo $_www_server?>/stat/query_saved.php'"><? echo trsLang('저장된 쿼리','saved_queries');?></li>
		...
		<form name="searchForm" action="<?php echo $_SERVER[PHP_SELF] ?>" method="POST">
			...
			<table  class="search">
					...
					<td style='padding:5px 13px'>
							...
							<option value="query_content" <?php if($searchopt == "query_content") { echo ' selected="selected"'; } ?>>
						...
						<input type="submit" value="<?= $_LANG_TEXT['usersearchtext'][$lang_code] ?>" class="btn_submit" onclick="return SearchSubmit(document.searchForm);">
							<input type="button" value="<? echo trsLang('초기화','btnclear');?>" class="btn_submit_no_icon" onclick="location.href='<? echo $_www_server?>/stat/query_saved.php'">
		...
		<div class="btn_wrap right" style='margin-bottom:10px;'>
			<? $excel_down_url = $_www_server."/stat/query_editor_excel.php?enc=".ParamEnCoding($param);?>
			<div class="right">
		...
		<table class="list" style="margin-top:10px; ">
			...
			<?php
				...
				while ($row = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC)) {
						...
						$utf8_query_content=utf8_strcut($query_content,150);
			...
			<tr >
				...
				<td class="center"><?=$query_title?></td>
				<td class="center" style="cursor:pointer;">	

				<form name="frmQuery" action="query_editor.php" method="post">
					  <input type="hidden" name="query_content" value="<?=$query_content?>">
					  <span onclick="event.stopPropagation(); this.parentNode.submit()" style='cursor:pointer;'>
                <?= $utf8_query_content;?>
            </span>
				...
				<td class="center"><?=$date_value?></td>
					<td class="center" onclick="event.stopPropagation();"><span onclick="queryDeleteSubmit('<?=$custom_query_seq?>')"
						style='cursor:pointer;'><?=$_LANG_TEXT['btndelete'][$lang_code]?></span></td>
