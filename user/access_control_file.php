...
<div id="user_list">
	<div class="container">
		...
		<div id="tit_area">
			<div class="tit_line">
				<h1><span id='page_title'><?= $_LANG_TEXT['access_control_theme'][$lang_code] ?>  <small><? echo trsLang('파일반입','fileimport');?></small></span></h1>
			</div>
		...
		<form name="searchForm" action="<?php echo $_SERVER[PHP_SELF] ?>" method="POST">
			...
			<table class="search">
				...
				$searchopt_list = array(
					...
					,"elec_doc_number"=>trsLang("전자문서번호","electronic_payment_document_number")."(".trsLang('출입번호','visitnumbertext').")"
				...
				<tr>
					...
					<td colspan="5" style='padding:5px 13px'>
						...
						<input name="uncovered" id="uncovered" type="submit" value="<?= trsLang('미회수조회','unrecovered_views'); ?>" style="cursor:pointer" class="btn_submit_no_icon" onclick="return SearchSubmit(document.searchForm);">
													<input type="button" value="<? echo trsLang('초기화','btnclear');?>" class="btn_submit_no_icon" onclick="location.href='<? echo $_www_server?>/user/access_control_file.php'">
			...
			<?php
			//검색항목
			 $search_sql = " and v3.label_value > '' ";	//반입이 있는 정보만 가져온다.(usb 물품관리번호)
			...
			$excel_name = $_LANG_TEXT['access_control_theme'][$lang_code]."(".trsLang('파일반입','fileimport').")";
			?>

			<div class="btn_wrap right" style='margin-bottom:10px;'>
				<? $excel_down_url = $_www_server . "/user/access_control_file_excel.php?enc=" . ParamEnCoding($param); ?>
				<div class="right">
		...
		<table class="list" style="margin-top:10px; ">
			<tr>
				...
				<th class="center" style='min-width:100px'><?= $_LANG_TEXT['importdatetimetext'][$lang_code] ?></th>
				...
				<th class="center" style='min-width::100px'><?= trsLang('전자문서번호','electronic_payment_document_number'); ?>(<? echo trsLang('출입번호','visitnumbertext');?>)</th>
