<?php
$_section_name = "pop_user_vcs_summary";
...
$param_enc = ParamEnCoding("src=".$_section_name."&v_user_seq=".$v_user_seq."&device_gubun=");
...
$("document").ready(function(){
	...
	StatisticsUserDeviceVcsData('<?=$v_user_seq?>');
});
</script>
<div id="mark">
	<div class="content">
...
?>	
			<div class='outline'>
				<div class="section04">
					...
					<div class="ch">
						<div class='wrap1'><div class='one'><div id='ChartAllData' class="txt2 cap3"><?=$str_all_cnt?></div><canvas id="ChartAll" width='250%' height='250%'/></canvas><div class="txt cap3"><?=$_LANG_TEXT["alltext"][$lang_code];?></div></div></div>
						<div class='wrap1'><div class='two'><div id='ChartNotebookData' class="txt2 cap1"><?=$str_notebook_cnt?></div><canvas id="ChartNotebook" width='250%' height='250%' /></canvas><div class="txt cap1" ><?=$_LANG_TEXT['laptoptext'][$lang_code]?></div></div></div>
						<div class='wrap1'><div class='three'><div id='ChartHddData' class="txt2 cap2"><?=$str_hdd_cnt?></div><canvas id="ChartHdd" width='250%' height='250%' /></canvas><div class="txt cap2" ><?=$_CODE['storage_device_type']['HDD']?></div></div></div>
						<div class='wrap1'><div class='four'><div id='ChartRemovableData' class="txt2 cap3"><?=$str_removable_cnt?></div><canvas id="ChartRemovable" width='250%' height='250%'/></canvas><div class="txt cap3"><?=$_CODE['storage_device_type']['Removable']?></div></div></div>
						<div class='wrap1'><div class='five'><div id='ChartEtcData' class="txt2 cap3"><?=$str_etc_cnt?></div><canvas id="ChartEtc" width='250%' height='250%'/></canvas><div class="txt cap3" ><?=$_CODE['storage_device_type']['DEVICE_ETC']?></div></div></div>
					</div>
				</div>
				<div class="section05">
					...
					<div class='ch'>
						<div class='wrap1'>
							<div class='one'>
								<div id='ChartWeakData' class="txt2 cap1" style='cursor:pointer' onclick="popUserVcsWeakVirus('<?=$v_user_seq?>','<?=$_section_name?>')"><?=$str_weak_cnt?></div>
						...
						<div class='wrap1'>
							<div class='two'>
								<div id='ChartVirusData' class="txt2 cap2" style='cursor:pointer' onclick="popUserVcsWeakVirus('<?=$v_user_seq?>','<?=$_section_name?>')"><?=$str_virus_cnt?></div>
			...
			<!--등록장비리스트-->
			<div id='user_device_vcs_list' style='margin-top:15px;height:500px;padding:5px;'></div>
