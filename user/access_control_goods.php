...
$page_tab_name = "access_control_goods";
...
<div id="user_list">
	<div class="container">
		...
		<div id="tit_area">
			<div class="tit_line">
				<h1><span id='page_title'><?= $_LANG_TEXT['access_control_theme'][$lang_code] ?>  <small><? echo trsLang('자산반입','bring_assets');?></small></span></h1>
			</div>
		...
		<form name="searchForm" action="<?php echo $_SERVER[PHP_SELF] ?>" method="POST">
			...
			<table class="search">
				...
				<tr>
					...
					<td colspan="5"  style='padding:5px 13px'>
						...
						<input type="button" value="<? echo trsLang('초기화','btnclear');?>" class="btn_submit_no_icon" onclick="location.href='<? echo $_www_server?>/user/access_control_goods.php'">
			...
			$excel_name = $_LANG_TEXT['access_control_theme'][$lang_code]."(".trsLang('자산 반입','bring_assets').")";
			?>
			<!-- for test -->

			<div class="btn_wrap right" style='margin-bottom:10px;'>
				<? $excel_down_url = $_www_server . "/user/access_control_goods_excel.php?enc=" . ParamEnCoding($param); ?>
				<div class="right">
		...
		<table class="list" style="margin-top:10px; ">
			<tr>
				...
				<th class="center" style='min-width:100px;max-width:200px;'><?= $_LANG_TEXT['importdatetimetext'][$lang_code] ?></th>
				<th class="center" style='min-width:100px'>
					<? echo trsLang('임직원','executives');?>
				</th>
				...
				<th class="center" style='min-width::100px'><?= $_LANG_TEXT['electronic_payment_document_number'][$lang_code] ?></th>
		...
		<?php
			if ($result) {
				while ($row = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC)) {
					...
					if($_encryption_kind=="1"){
					 ...
					 $str_in_time=setDateFormat($row['in_time'],"Y-m-d");
			...
			<tr>
				...
				<td class="center" ><?= $no ?></td>
					...
					<?= $manager_name ?> <? if($manager_name_en !="") echo "({$manager_name_en})"; ?></a></td>
